            ...
            <form method="POST" action="{{ route('issues.update', $issue) }}" enctype="multipart/form-data">
                ...
                <div class="px-6 py-4">
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" id="title" value="{{ $issue->title }}" class="w-full border-gray-300 rounded-md shadow-sm" required>
                    </div>
                    <div class="mb-4">
                        <label for="description" class="block text-gray-700 dark:text-gray-200">Description</label>
                        <textarea name="description" id="description" rows="5" class="w-full border-gray-300 rounded-md shadow-sm" required>{{ $issue->description }}</textarea>
                    </div>
                    <div class="mb-4">
                        ...
                        <select name="category_id" id="category_id" class="w-full border-gray-300 rounded-md shadow-sm">
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" @if($issue->category_id == $category->id) selected @endif>{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
